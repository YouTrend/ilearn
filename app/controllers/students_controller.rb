class StudentsController < ApplicationController
	...
	def index
      @students = Student.studying.page(params[:page] || 1).per(20)
      @destroy_courses_flag = true
	end

	def search_auto_complete
			render json: Student.term_for(params[:term]).pluck(:name)
	end
	...
	def others
      @students = Student.others.page(params[:page] || 1).per(20)
	...
	def student_params
		params.require(:student).permit(:afts_id, :card_id, :school, :grade, :name, :birthday, :identity_code, :address, :remark, :course_ids => [], :contacts_attributes => [:id, :name, :phone, :phone2, :phone3, :notify_demand, :line_token])
	end	
end